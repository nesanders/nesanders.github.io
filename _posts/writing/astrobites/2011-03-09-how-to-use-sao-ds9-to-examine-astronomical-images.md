...
As a sample image, we'll use the [Sloan Digital Sky Survey](http://data.sdss3.org/fields.html?search=radec&ra=12%3A39%3A59.4s&dec=+-11%3A37%3A23s&submit=Submit)'s (SDSS) g-band image of the Sombrero galaxy ([M104](http://nedwww.ipac.caltech.edu/cgi-bin/nph-objsearch?objname=sombrero&extend=no&hconst=73&omegam=0.27&omegav=0.73&corr_z=1&out_csys=Equatorial&out_equinox=J2000.0&obj_sort=RA+or+Longitude&of=pre_text&zv_breaker=30000.0&list_limit=5&img_stamp=YES)), which you can [download here](http://data.sdss3.org/returnIms/fits?run=6005&camcol=6&field=85&filter=g).  Download this FITS image and open it via the File > Open menu item in ds9.  You can go to File > Display FITS Header to learn everything you could ever want to know about how this SDSS image was made.
...
When you move your cursor around, also notice that the current [RA/DEC position](http://curious.astro.cornell.edu/question.php?number=112) of your cursor is displayed in the text boxes in the upper-left corner.   Note that ds9 can only calculate these coordinates if your FITS image has a [World Coordinate System](http://fits.gsfc.nasa.gov/fits_wcs.html) (WCS) specified, as all the SDSS images do.  In either case, ds9 will also display the pixel (x,y) coordinates.
...
With your g-band image still open, download the [SDSS u-band image of the Sombrero galaxy](http://data.sdss3.org/returnIms/fits?run=6005&camcol=6&field=85&filter=u).  Now go to Frame > New Frame.  You can think of a frame in ds9 like a tab in a web browser - it's just a container to open images in.  With the new frame open, go to File > Open to open the u-band image you downloaded.
...
You should notice that the sources that are bright in the g-band image are dimmer in the u-band and that many faint sources are totally undetectable in the u-band.  This is due to a combination of factors that conspire to make astronomical sources dim in the UV, which the u-band is [just on the edge of](http://cadcwww.dao.nrc.ca/megapipe/docs/cfhtlsugriznew.gif), including atmospheric opacity, the sensitivity of CCDs, and the [effective temperature](http://upload.wikimedia.org/wikipedia/commons/a/a2/Wiens_law.svg) of different sources.

If you download one more image, the [r-band image](http://data.sdss3.org/returnIms/fits?run=6005&camcol=6&field=85&filter=r), you can make a color mosaic!  Go to Frame > New Frame RGB.  A new frame should open, along with a popup window titled "RGB."  You use this window to essentially access the three subframes (R, G, and B) within this color frame.  First click on the "Red" option in the popup and use File > Open to open the r-band image.  Then click on "Green" and open the g-band, and then "Blue" and open the u-band.  You'll have to play with the colormap in each color frame to make something as pretty as the [SDSS mosaic](http://data.sdss3.org/fullJpg?run=6005&camcol=6&field=85).
...
While you have your color mosaic up, let's check the SDSS optical data against an infrared catalog.  You would only expect the sources that are bright and red to show up in the infrared data.  To automatically load the catalog of sources near the Sombrero galaxy from the [Two Micron All Sky Survey](http://www.ipac.caltech.edu/2mass/) (2MASS), go to Analysis > Catalogs > Infrared > 2MASS Point Sources.  ds9 will automatically draw circles around all the infrared sources in your image and also open a table listing them.  Click on an object in the table to pan over to it on the image.  You could even download the actual image from 2MASS by going to Analysis > Image Servers > IPAC-2MASS
...
All of the functions described above are extremely useful for interactively examining data, but it's also possible to automate (script) actions in ds9.  The simplest way is to pass [command line options](http://hea-www.harvard.edu/RD/ds9/ref/command.html) to ds9 when you first load it.  You can also use the [XPA messaging system](http://hea-www.harvard.edu/RD/ds9/ref/xpa.html) (or its [python interface](http://hea-www.harvard.edu/saord/ds9/pyds9/)) to pass commands to an open instance of ds9.

Of course, some of ds9's most powerful and frequently-used functionality comes from its capacity as a [display server](http://hea-www.harvard.edu/RD/ds9/ref/iraf.html) for [IRAF](http://iraf.net/).  ds9 can also interact with the table management software [TOPCAT](http://www.star.bris.ac.uk/~mbt/topcat/).
